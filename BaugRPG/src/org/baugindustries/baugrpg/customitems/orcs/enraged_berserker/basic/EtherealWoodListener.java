...
public class EtherealWoodListener implements Listener {
	private Main plugin;
	
	public EtherealWoodListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(BlockPlaceEvent event) {
		if (!Recipes.ETHEREAL_WOOD.matches(plugin, event.getItemInHand())) return;
