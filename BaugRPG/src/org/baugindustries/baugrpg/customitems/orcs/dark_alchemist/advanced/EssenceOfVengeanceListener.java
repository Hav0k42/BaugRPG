...
public class EssenceOfVengeanceListener implements Listener {
	private Main plugin;
	
	public EssenceOfVengeanceListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(PlayerInteractEntityEvent event) {
		...
		if (event.getHand().equals(EquipmentSlot.HAND)) {
			if (!Recipes.ESSENCE_OF_VENGEANCE.matches(plugin, player.getInventory().getItemInMainHand())) return;
		} else {
			if (!Recipes.ESSENCE_OF_VENGEANCE.matches(plugin, player.getInventory().getItemInOffHand())) return;
		}
