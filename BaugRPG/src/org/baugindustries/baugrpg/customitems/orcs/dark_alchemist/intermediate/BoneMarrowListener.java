...
public class BoneMarrowListener implements Listener {
	private Main plugin;
	
	public BoneMarrowListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(PlayerInteractEntityEvent event) {
		...
		if (event.getHand().equals(EquipmentSlot.HAND)) {
			if (!Recipes.BONE_MARROW.matches(plugin, player.getInventory().getItemInMainHand())) return;
		} else {
			if (!Recipes.BONE_MARROW.matches(plugin, player.getInventory().getItemInOffHand())) return;
		}
