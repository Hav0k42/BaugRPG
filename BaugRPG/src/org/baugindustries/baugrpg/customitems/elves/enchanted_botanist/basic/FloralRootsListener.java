...
public class FloralRootsListener implements Listener {
	private Main plugin;
	
	public FloralRootsListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(BlockPlaceEvent event) {
		if (!Recipes.FLORAL_ROOTS.matches(plugin, event.getItemInHand())) return;
