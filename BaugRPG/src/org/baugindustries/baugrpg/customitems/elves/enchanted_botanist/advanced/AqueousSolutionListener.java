...
public class AqueousSolutionListener implements Listener {
	private Main plugin;
	
	public AqueousSolutionListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(PlayerInteractEntityEvent event) {
		...
		if (event.getHand().equals(EquipmentSlot.HAND)) {
			if (!Recipes.AQUEOUS_SOLUTION.matches(plugin, player.getInventory().getItemInMainHand())) return;
		} else {
			if (!Recipes.AQUEOUS_SOLUTION.matches(plugin, player.getInventory().getItemInOffHand())) return;
		}
