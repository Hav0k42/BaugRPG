...
public class LunarDebrisListener implements Listener {
	private Main plugin;
	
	public LunarDebrisListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(BlockPlaceEvent event) {
		if (!Recipes.LUNAR_DEBRIS.matches(plugin, event.getItemInHand())) return;
