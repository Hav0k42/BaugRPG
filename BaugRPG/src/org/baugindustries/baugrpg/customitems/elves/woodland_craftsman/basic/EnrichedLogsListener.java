...
public class EnrichedLogsListener implements Listener {
	private Main plugin;
	
	public EnrichedLogsListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(BlockPlaceEvent event) {
		if (!Recipes.ENRICHED_LOGS.matches(plugin, event.getItemInHand())) return;
