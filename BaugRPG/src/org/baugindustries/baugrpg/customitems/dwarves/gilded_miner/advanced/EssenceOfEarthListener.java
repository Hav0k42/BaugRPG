...
public class EssenceOfEarthListener implements Listener {
	private Main plugin;
	
	public EssenceOfEarthListener(Main plugin) {
		this.plugin = plugin;
	}
	...
	public void onInventoryClick(PlayerInteractEntityEvent event) {
		...
		if (event.getHand().equals(EquipmentSlot.HAND)) {
			if (!Recipes.ESSENCE_OF_EARTH.matches(plugin, player.getInventory().getItemInMainHand())) return;
		} else {
			if (!Recipes.ESSENCE_OF_EARTH.matches(plugin, player.getInventory().getItemInOffHand())) return;
		}
